...
router.post('/insertdt', async(req, res) => {
    ...
    try {
        ...
        const [result] = await connection.query(`INSERT INTO registrations (Id, User_Name, Name, Age, phone, Email, Gender, Category, Duty_time, ${tableToJoin})
        SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, 
        FROM ${tableToJoin}
        WHERE ${tableToJoin}.Id = ?`, [Id, User_Name, Name, Age, phone, Email, Gender, Category, Duty_time]);
