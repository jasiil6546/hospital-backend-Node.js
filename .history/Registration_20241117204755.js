...
app.post('/booking', (req, res) => {
    ...
    console.log(Name, Age, phone, Email, Gender, Address);


    connection.query('INSERT INTO patients(Name, Age, phone, Email, Gender, Address) VALUES (?,?,?,?,?,?)', [Name, Age, phone, Email, Gender, Address], (error, results) => {
        if (error) {
            console.error('Error booking:', error);
            return res.status(500).json({ error: 'Database error, booking failed' });
        }
        res.status(201).json({ message: 'Booking successful', data: results });
    });
});
//////delete booking
pp.post('/delbooking', (req, res) => {
            ...
            connection.query('DELETE FROM patients WHERE phone = ?', [phone], (error, results) => {
                        ...
                        if (results.affectedRows === 0) {
                            // No rows affected, meaning no matching phone number found
                            return res.status(404).json({ message: 'No booking found for the given phone number' });
                        }
                        app.listen(3000);